...
test('Boolean false request schemas reject every request and no handler executes', async (t) => {
  ...
  fastify.post('/body', {
    handler: async () => {
      bodyExecuted = true
      return { protectedActionExecuted: true }
    },
    schema: { body: false }
  })
...
test('FSTWRN001 is emitted only for explicitly undefined schemas, not boolean false', async (t) => {
  ...
  // Only the explicitly `undefined` body schema warns; the `false` one is a
  // valid deny-all schema and must be compiled instead of warned about.
  t.assert.strictEqual(spyData.callCount(), 1)
  t.assert.strictEqual(
    spyData.calls[0].message,
    FSTWRN001.format('body', 'POST', '/undefined-body')
  )
